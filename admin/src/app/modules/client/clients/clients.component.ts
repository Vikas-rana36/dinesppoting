...
export class ClientsComponent implements OnInit {
  ...
  _fetchListing(){
    ...
    this.utilsService.processPostRequest('/client/listing',{}).pipe(takeUntil(this.destroy$)).subscribe((results:any) => {   
  ...
  cancelEdit(){
    ...
    this.formStatus = 'Add'
    this.clientForm.patchValue({is_active:true});
  ...
  onSubmit(){
    ...
    this.utilsService.processPostRequest('/client/add',this.clientForm.value).pipe(takeUntil(this.destroy$)).subscribe((response) => {
  ...
  delete(record:any){
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.utilsService.showPageLoader(environment['MESSAGES']['SAVING-INFO']);//show page loader
        this.utilsService.processPostRequest('/client/delete',{id:record._id}).pipe(takeUntil(this.destroy$)).subscribe((response) => {
          if(response){
  ...
  edit(record:any){
    // console.log(record)
     this.clientForm.patchValue({ id: record._id})
     this.clientForm.patchValue({ name: record.name})
   ...
   cancel(){
    this.clientForm.reset();
    this.clientForm.patchValue({ id: null, is_active:true})
    this.isCollapsed = false;
    this.formStatus = 'Add'
  }
